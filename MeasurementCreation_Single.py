...
def save(Wavelenghts, Intensities, Serial, Notes):
    ...
    FileName = f"data/spectra/{TIME}_{Serial}_{Wavelength}_Notes.csv"
...
def Main():
    ...
    try:
        ...
        for i in range(Config['acquisition']['runs_to_make']):
            print(f"Running scan {i+1}/{Config['acquisition']['runs_to_make']}")
            Spectrometer.open()
            ...
            save(Wavelengths, Intensities, Spectrometer.serial_number, Config['acquisition']['notes'])
            Spectrometer.close()
        print("Measurements completed, please start with the analysis.")
